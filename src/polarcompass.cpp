...
void TacticsInstrument_PolarCompass::Draw(wxGCDC* bdc)
{
    ...
    if (!wxIsNaN(m_Bearing) && !wxIsNaN(m_ExtraValueDTW)){
      //DrawData(bdc, m_Bearing, m_BearingUnit, wxT("BRG:%.f"), DIAL_POSITION_TOPLEFT);
  ...
  if (!wxIsNaN(m_TWS)) {
    ...
    if (!wxIsNaN(m_Bearing)){
      if (m_Bearing >= 0 && m_Bearing < 360 && !wxIsNaN(m_TWD)){
        ...
        if (!wxIsNaN(TCMGMax.TargetAngle))      DrawTargetAngle(dc, TCMGMax.TargetAngle, wxT("URED"), 2);
        if (!wxIsNaN(TCMGMin.TargetAngle))      DrawTargetAngle(dc, TCMGMin.TargetAngle, wxT("URED"), 1);
      }
    }
  }
}
...
bool TacticsInstrument_PolarCompass::LoadConfig(void)
{
	...
	if (pConf) {
		pConf->SetPath(wxT("/PlugIns/Tactics/PolarCompass"));
